...
function love.keypressed(key)
  if key=="p" then fullscreen=not fullscreen love.window.setFullscreen(fullscreen) end
  if key=="r" then resetworld() end
  if key=="space" then  replacetile(round(mxworldpos),round(myworldpos),{["tile"]="stone",["top"]="none",["back"]="none",["light"]={1,1,1,1}}) end
  ...
  if key=="e" and spectator then camv=camv+8 if camv>96 then camv=96 end end
  if key=="q" and spectator then camv=camv-8 if camv<8 then camv=8 end end
end
